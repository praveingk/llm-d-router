...
*   **Weighted LAS** (`weighted-las-fairness-policy`): Least-attained-service scheduling in which each flow's attained service is divided by a share weight declared in the `x-llm-d-inference-fairness-weight` header, so service shares converge on the ratio of those weights. Configuration is documented in the doc comment of [weightedlas.go](./weightedlas/weightedlas.go).
